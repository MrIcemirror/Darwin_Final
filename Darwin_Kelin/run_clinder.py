...
for i in range(len(params)):
    ...
    gen_radius = float(gen_radius_python.split("*")[0])/1000
    gen_z = float(gen_z_python.split("*")[0])
    area = 2*pi*gen_radius**2+2*pi*gen_radius*gen_z
    test_number_python = str(int(area*flux*test_number_python))
    ...
    for j in range(1):
        ...
        os.system("cd build && make && ./Darwin_shielding_main ")
    #break
